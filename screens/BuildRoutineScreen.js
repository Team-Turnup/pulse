...
class BuildRoutineScreen extends Component {
  ...
  handleChange(key, value) {
    ...
    if (key==='routineType' && value!=='combo'){
      const newRoutine = this.state.routine.map(interval => ({cadence: interval.cadence, duration: interval.duration, intervalType: value}))
  ...
  addInterval() {
    ...
    const newRoutine = [...routine.slice(0, index+1), {cadence, duration, intervalType}, ...routine.slice(index+1)]
    this.setState({routine: newRoutine, index: routine.length===0 ? index : index+1})
  }

  saveInterval() {
    const {cadence, duration, routine, index, intervalType} = this.state
    const newRoutine = [...routine.slice(0, index), {cadence, duration, intervalType}, ...routine.slice(index+1)]
  ...
  changeIndex(index) {
    ...
    this.setState({index, cadence: interval.cadence, duration: interval.duration, intervalType: interval.intervalType})
  }
    ...
    return (
      <Container>
        ...
        <Content>
            ...
            <Label>Activity Type</Label>
            <RNPickerSelect
              onValueChange={value => this.handleChange('intervalType', value)}
          ...
          <TouchableOpacity
            ...
            onPress={() => this.state.intervalType ? this.addInterval() : {}}
          >
            <Text style={styles.buttonText}>Insert Next Interval</Text>
          </TouchableOpacity>

          {this.state.index<this.state.routine.length ? <TouchableOpacity
             style={{...styles.button, backgroundColor: this.state.intervalType ? 'blue' : 'gray'}}
            onPress={() => this.state.intervalType ? this.saveInterval() : {}}
          >
            <Text style={styles.buttonText}>Save Changes to Current Interval</Text>
          ...
          >
            <Text style={styles.buttonText}>Remove Current Interval</Text>
          </TouchableOpacity> : null }
